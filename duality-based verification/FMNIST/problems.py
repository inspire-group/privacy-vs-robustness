...
def fmnist_loaders(batch_size=20, test_batch_size=2): 
    fmnist_train = datasets.FashionMNIST("../../datasets/fmnist", train=True,
       download=True, transform=transforms.ToTensor())
    fmnist_test = datasets.FashionMNIST("../../datasets/fmnist", train=False,
       download=True, transform=transforms.ToTensor())
    train_loader = torch.utils.data.DataLoader(fmnist_train, batch_size=batch_size, shuffle=True, pin_memory=True)
    test_loader = torch.utils.data.DataLoader(fmnist_test, batch_size=test_batch_size, shuffle=False, pin_memory=True)
...
def argparser(batch_size=50, epochs=20, seed=0, verbose=1, lr=1e-4, 
    ...
    parser.add_argument('--width_num', type=int, default=16)
