...
class Top(nn.Module):
    ...
    def boxSpec(self, x, target):
        ...
        ttt = min(max(600,self.count-20*600), self.schedule)/self.schedule
...
parser.add_argument('--width', type=float, default=0.1, metavar='CW', help='the width of either the line or box')
...
parser.add_argument('-n', '--net', choices = h.getMethodNames(models), action = 'append'
                    , default=['FMNIST_VGG'], help='picks which net to use for training')  # one net for now

parser.add_argument('-D', '--dataset', choices = [n for (n,k) in inspect.getmembers(datasets, inspect.isclass) if issubclass(k, Dataset)]
                    , default="FMNIST", help='picks which dataset to use.')

parser.add_argument('-o', '--out', default="out/", help='picks which net to use for training')
parser.add_argument('--test-size', type=int, default=5000, help='number of examples to test with')
...
args.log_interval = int(60000 / (args.batch_size * args.log_freq))
